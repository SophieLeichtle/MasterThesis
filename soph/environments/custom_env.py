...
class CustomEnv(BaseEnv):
    """
    Custom iGibson Environment (OpenAI Gym interface)
    Modified version of igibson_env.py
    """
    ...
    def load_task_setup(self):
        ...
        if "task" not in self.config:
            ...
            raise Exception("Given Task is undefined.")

        # subtasks
        # a subtask will execute its step every frame but not terminate the task
        self.subtasks = {}
        if "subtasks" in self.config:
            if "occupancy" in self.config["subtasks"]:
                self.subtasks["occupancy"] = OccupancySubtask(self)
            if "scanning" in self.config["subtasks"]:
                self.subtasks["scanning"] = ScanSubtask(self)
    ...
    def step(self, action):
        ...
        for key in self.subtasks:
            self.subtasks[key].step(self)
            info[key] = self.subtasks[key].get_task_obs(self)
