...
def save_map(log_dir, robot_state, map, detection_tool, current_plan = [], frontier_line = []):
    ...
    cv2.circle(map_cv, [int(robot_pos_in_map[1]), int(robot_pos_in_map[0])], radius=base_radius_in_map, color=(255,0,0), thickness=-1)
    for def_det in detection_tool.definitive_detections:
        det_pos_in_map = map.m_to_px(def_det.position)
        cv2.circle(map_cv, [int(det_pos_in_map[1]), int(det_pos_in_map[0])], radius=3, color = (0, 255, 0), thickness=-1)
...
def save_nav_map(log_dir, map, nav_graph):
    ...
    for node in nav_graph.nodes:
        if node.parent is not None:
            ...
            cv2.line(map_cv, [int(pos_in_map[1]), int(pos_in_map[0])], [int(parent_in_map[1]), int(parent_in_map[0])],color=(0,255,0), thickness=1)
            cv2.circle(map_cv, [int(pos_in_map[1]), int(pos_in_map[0])], radius=2, color=(0,255,0), thickness=-1)
    root_in_map = map.m_to_px(nav_graph.root.position)
    cv2.circle(map_cv, [int(root_in_map[1]),int(root_in_map[0])], radius=3, color=(255,0,0), thickness=-1)
    cv2.imwrite(os.path.join(log_dir, filename), map_cv)

def save_map_combo(log_dir, robot_state, map, detection_tool, nav_graph, current_plan = [], frontier_line = []):
    ...
    for def_det in detection_tool.definitive_detections:
        det_pos_in_map = map.m_to_px(def_det.position)
        cv2.circle(map_cv, [int(det_pos_in_map[1]), int(det_pos_in_map[0])], radius=3, color = (0, 127, 255), thickness=-1)
    # POIs: Red
    for poi in detection_tool.pois:
        poi_in_map = map.m_to_px(poi[:2])
        cv2.circle(map_cv, [int(poi_in_map[1]), int(poi_in_map[0])], radius=3, color=(0, 0, 255), thickness=-1)
    ...
    for node in nav_graph.nodes:
        if node.parent is not None:
            ...
            cv2.circle(map_cv, [int(pos_in_map[1]), int(pos_in_map[0])], radius=2, color=(0,255,0), thickness=-1)
    root_in_map = map.m_to_px(nav_graph.root.position)
    cv2.circle(map_cv, [int(root_in_map[1]),int(root_in_map[0])], radius=3, color=(0,127,0), thickness=-1)
    cv2.imwrite(os.path.join(log_dir, filename), map_cv)