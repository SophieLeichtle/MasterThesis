...
def sample_plan_poi(env, map, poi, base_radius=DEFAULT_FOOTPRINT_RADIUS, n=10, verbose=False):
    if verbose: logging.info(poi)
    if map.check_if_free(poi[:2], base_radius):
        return plan_base_motion(env.robots[0], poi, map, robot_footprint_radius=base_radius)
    ...
    for r in r_samples:
        for c in c_samples:
            ...
            if map.check_if_free(sample[:2], base_radius):
                if verbose: logging.info(sample)
...
def frontier_plan_poi(env, map, poi, base_radius=DEFAULT_FOOTPRINT_RADIUS, verbose=False):
    ...
    while current_plan is None and len(frontier_lines) > 0:
        ...
        for s in samples:
            ...
            s[2] = poi[2]
            if verbose: logging.info(s)
            plan = plan_base_motion(env.robots[0], s, map, robot_footprint_radius=base_radius)
            if plan is None: continue
            current_plan = plan
            break
        if current_plan is None:
