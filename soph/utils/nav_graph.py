...
class NavGraph:
        ...
        for node in self.nodes:
            ...
            if dist < best_dist:
                if map is not None:
                    ...
                    if not map.line_of_sight(node_in_map, pos_in_map): continue
                closest_node = node
                best_dist = dist
        return closest_node
    ...
    def rewire_two_nodes(self, base_node, node_to_rewire):
        current_cost = base_node.get_cost() + np.linalg.norm(base_node.position - node_to_rewire.position)
    ...
    def update_with_robot_pos(self, robot_pos, map, min_dist=1.0):
        ...
        if closest_node is not self.root and closest_node is not None:
            ...
            if closest_dist < dist:
                ...
                logging.info("Moving Root to " + f'{closest_node.position[0]:.2f}, {closest_node.position[1]:.2f}')
                self.move_root(closest_node)
                dist = closest_dist

        if dist > min_dist:
            logging.info("Adding nav_node at " + f'{robot_pos[0]:.2f}, {robot_pos[1]:.2f}')
